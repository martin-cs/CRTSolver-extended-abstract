...
Time and successful solving are the benchmarks used to evaluate the performance of CRTSolver 
in both available modes on non-linear integer equations, in comparison with two
widely used and state-of-the-art SMT solvers, Z3 \cite{10.1007/978-3-540-78800-3_24} and cvc5.
Time simply refers to the total runtime for a given equation in milliseconds. 
We define a successful solving of a given
...
In Table \ref{table:results} we give a comparison with Z3 and cvc5.
...
groups: equations with 1 variable, equations with 2 variables and equations with 3 variables.
All equations in the first 2 groups contained only a single assert statement, whereas the third group
contained a mixture of equations with single assert statements, and equations with multiple assert statements.
Each of these 3 groups contained sub-groups of quadratic and cubic equations. There was a mixture of both 
...
For each SMT-LIB file, we give the number of variables present, the degree, the number of equations and whether or not the file is 
...
These results suggest that there is a realistic potential in using CRTSolver in Bit-Vector Mode
...
%
Z3 has excellent performance because of a preprocessing step that 
factors polynomials (using modulo arithmetic and lifting).  In the
